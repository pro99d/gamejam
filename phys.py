...
class Window(arcade.Window):
    def __init__(self):
        ...
        self.a = Vec2(500, 350)
        self.b = Vec2(180, 350)
        a = (self.b-self.a).angle(Vec2(1, 0))
        self.norm = Vec2(math.cos(a), math.sin(a))
    def collide(self):
        angle = self.norm.angle(Vec2(1, 0))-2*(self.vel.angle(self.norm) - self.norm.angle(Vec2(1, 0)))
        
        d = math.degrees(angle)
        cos = math.cos(angle)
        sin = math.sin(angle)
        self.vel.x = self.vel.x * cos - self.vel.y * sin
        self.vel.y = self.vel.x * sin + self.vel.y * cos 
